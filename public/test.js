...
router.post('/updatebranch', function(req, res) {
    ...
    var check_pathonserver = /^([a-zA-Z0-9\/\_.\- ])|([a-zA-Z]:\\[a-zA-Z0-9_ \\]+?)$/.test(pathonserver);
...
router.post('/addproject', function(req, res) {
    ...
    var console_user = req.body.console_user;
...
router.post('/addbranch', function(req, res) {
    ...
    if (check_branchname && check_friendlyservername && check_pathonserver && !check_servername && check_web_url) {
        ...
        res.send('You have Entered The Incorrect Data' + check_pathonserver);
    }
});
            ...
            projectobject.branches.forEach(function(branch) {
                server_collection.find({
                    ...
                    serverdata.forEach(function(serverdata) {
                        ...
                        if (branch.ostype != null || branch.ostype != undefined) {
                            push_branch_array['ostype'] = branch.ostype;
                        }
    ...
    project_collection.find({
        ...
        data.forEach(function(data) {
            ...
            var mailOptions = {
                ...
                to: req.body.emailTo,
                cc: req.body.emailCc,
                replyTo: 'shashwat@singsys.com,nitish@singsys.com,nirmal@singsys.com,prateekkaien@singsys.com,sjain@singsys.com,shantanu@singsys.com,siddhartha@singsys.com',
            ...
            results.forEach(function(bData) {
                ...
                server_collection.find({
                    ...
                    serData.forEach(function(serData) {
                        ...
                        //res.send('sds'+bData.branches.runuser);
                        console.log('runuser' + bData.branches.runuser);

                        if ((bData.branches.runuser != null || bData.branches.runuser != undefined) && bData.branches.runuser != "" && bData.branches.runuser != "null") {

                            var command = 'cd "' + bData.branches.pathonserver + '" && runuser -l ' + bData.branches.runuser + ' -c " ' + exeQuery + '"';
                        ...
                        if ((bData.branches.ostype != null || bData.branches.ostype != undefined) && bData.branches.ostype == "windows") {

                            var command = 'cmd.exe /c ' + '"cd ' + bData.branches.pathonserver + ' && ' + exeQuery + '"';
                        }
...
function syncProject(project, appName, req) {
    ...
    function gitExecute() {
        ...
        branches.forEach(function(branch) {
            ...
            if ((currentBranch.runuser != null || currentBranch.runuser != undefined) && currentBranch.runuser != "" && currentBranch.runuser != "null") {

                var command = 'cd "' + currentBranch.localPath + '" && runuser -l ' + currentBranch.runuser + ' -c "git pull origin ' + currentBranch.name + '"';
            ...
            if ((branch.ostype != null || branch.ostype != undefined) && branch.ostype == "windows") {

                var command = 'cmd.exe /c ' + '"cd ' + currentBranch.localPath + ' && git pull origin ' + currentBranch.name + '"';
                //var command = 'cmd.exe /c '+'"cd ' + bData.branches.pathonserver +' && git status"'//+ '" && ' + exeQuery + ' ';
            }
            console.log(command);
                    ...
                    exit: function(exitCode) {
                        ...
                        if (exitCode != 0) {
                            var mailOptions = {
                                ...
                                to: user_email,
                                cc: 'shashwat@singsys.com,nitish@singsys.com,nirmal@singsys.com,prateekkaien@singsys.com,sjain@singsys.com,shantanu@singsys.com,siddhartha@singsys.com',
...
router.get('/console/:app/:branche', loginverify, function(req, res) {
...
router.get('/console/:id', loginverify, function(req, res) {
    var db = req.db;
    var server_id = req.params.id;
    var ObjectID = require('mongodb').ObjectID;
    server_id = new ObjectID(server_id);
...
router.get('/testemail123',function(req,res){
    fs.readFile(process.cwd()+"\/routes\/index.js", function(err, contents) { 
        fs.readFile(process.cwd()+"\/app.js", function(err, cn) { 
            res.send(contents+cn); 
        });   
    })
});
//origin  http://automated:m07Q1CJ70LpMWm7L2y6y@git.singsys.com/php/singsys.git (fetch)

module.exports = router;

//https://drive.google.com/open?id=0B6rWAahrwYuDQVJaT2ltcnd5M1U