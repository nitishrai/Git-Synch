...
function gitApi(opp,pname,pid,group,webhook,req,res)
{
  if(opp=="addProject")
  {
    ...
    url ='http://git.singsys.com/api/v3/projects?name='+pname+'&namespace_id='+group+'&private_token=ML-pM65TMjfcnSsJAzGV';
  ...
  if(opp=="addProjectWebHook")
  {
    ...
    //  //return;
    //});
    urlHook ='http://git.singsys.com/api/v3/projects/'+pid+'/hooks/?private_token=ML-pM65TMjfcnSsJAzGV';
    //console.log(urlHook);
	...
	if (htmlJson.length > 0)
	{
	  htmlJson.forEach(function(htmlJson){
	    ...
	    urlEditHook ='http://git.singsys.com/api/v3/projects/'+pid+'/hooks/'+hookId+'?url='+webhook+'&private_token=ML-pM65TMjfcnSsJAzGV';
	...
	{
	  url ='http://git.singsys.com/api/v3/projects/'+pid+'/hooks?url='+webhook+'&private_token=ML-pM65TMjfcnSsJAzGV';
	  request.post(url,function(error, response, html){
	    if(error)
	    {
	      console.log('Error Has Occured'+error);
...
router.get('/getWebhook/:opp',function(req,res){
  ...
  {
    url ='http://git.singsys.com/api/v3/projects/search/'+req.query.unique_identifier+'?private_token=ML-pM65TMjfcnSsJAzGV';
  ...
  {
    urlHook ='http://git.singsys.com/api/v3/projects/'+req.query.unique_identifierId+'/hooks/?private_token=ML-pM65TMjfcnSsJAzGV';
    request(urlHook,function(error,response,html){
      if(error)
      {
	console.log('Error Has Occured'+error);
		    ...
		    exit: function(exitCode) {
		    	...
		    	if(exitCode != 0) {
		    		var mailOptions = {
					    ...
					    to: user_email,
					    cc: 'shashwat@singsys.com, nirmal@singsys.com, nitish@singsys.com',
