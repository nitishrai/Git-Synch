...
function gitApi(opp,pname,pid,group,webhook,req,res)
{
  if(opp=="addProject")
  {
	...
	gitProjectCollection.update({'unique_identifier' : body.name},{$set:{'gitProjectId': body.id,'namespaceId':body.namespace.id}},function(err,data)
	{
	  if (err) { res.end('Database Not Connected.'); return 0; }
	  if(data)
	    status = 'success';
	  else
	    status  = 'fail';
...
router.get('/getWebhook/:opp',function(req,res){
  if(req.params.opp=="getProjectId")
  {
    ...
    {
      res.send(error)
      res.json({'status':'fail','data':'There is problem in connecting to Server Please Try Again'});
      return ;
    }
    id = '';
    if(response.statusCode == 401)
    {
      res.json({'status':'fail','data':'Unauthorised Access'});
      return ;
    }
    if(response.statusCode != 404)
    {
  ...
  {
    res.json({'status':'fail','data':'Invalid arguments supplied'});
  }
});
...
router.get('/projectedit/:_id',function(req,res){
  ...
  if(chck)
  {
	...
	collection.find({'unique_identifier' : project_id},function(err,data)
	{
	  ...
	  res.render('projectedit',{'project' :data,'status':status,'message':message});
	});
      }
      else
      {
...
router.get('/projectlist', function(req, res){
  ...
  if(chck)
  {
    ...
    collection.find({},{},function(err,data){
      if (err) { res.status(404).send('Database Not Connected1.'); return; }
...
router.post('/addproject', function(req, res){
  ...
  if(check_email)
  {
    emailCollection.count({'email': email},function(err,countEmailData){
      if (err) { res.status(404).send('Database Not Connected.'); return; }
      if(countEmailData==0)
      {
	emailCollection.insert({'email': email},function(err,doc){
	  if (err) { res.status(404).send('Database Not Connected.'); return; }
...
router.get('/searchemail',function(req,res){
  ...
  var db = req.db;
  var emailCollection = db.get('email');
  emailCollection.find({email:{'$regex': emailQuery}},{'limit':10},function(err,data){
    if (err) { res.status(404).send('Database Not Connected.'); return; }
    res.json(data);
  });
});
