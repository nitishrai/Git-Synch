...
function gitApi(opp, pname, pid, group, webhook, req, res) {
    if (opp == "addProject") {
        ...
        url = 'http://git.singsys.com/api/v3/projects?name=' + pname + '&namespace_id=' + group + '&private_token=hNaPq1UPYBmzRkKK25yH';
    ...
    if (opp == "addProjectWebHook") {
        //urlFindHook = 'http://localhost:'+req.app.settings.port+'/getWebhook/getProjectHook?unique_identifierId='+pid;
        //request({url:urlFindHook, json: true},function(error,response,html){
        //  //res.end('dsfds');
        //  //console.log(error);
        //  //res.send(response);     
        //  res.send(html);
        //  //return;
        //});
        urlHook = 'http://git.singsys.com/api/v3/projects/' + pid + '/hooks/?private_token=hNaPq1UPYBmzRkKK25yH';
        //console.log(urlHook);
            ...
            if (error) {
                ...
                if (htmlJson.length > 0) {
                    htmlJson.forEach(function(htmlJson) {
                        ...
                        urlEditHook = 'http://git.singsys.com/api/v3/projects/' + pid + '/hooks/' + hookId + '?url=' + webhook + '&private_token=hNaPq1UPYBmzRkKK25yH';
                    ...
                    url = 'http://git.singsys.com/api/v3/projects/' + pid + '/hooks?url=' + webhook + '&private_token=hNaPq1UPYBmzRkKK25yH';
                    request.post(url, function(error, response, html) {
                        if (error) {
                            console.log('Error Has Occured' + error);
                            return;
...
router.get('/getWebhook/:opp', function(req, res) {
    if (req.params.opp == "getProjectId") {
        url = 'http://git.singsys.com/api/v3/projects/search/' + req.query.unique_identifier + '?private_token=hNaPq1UPYBmzRkKK25yH';
        ...
        urlHook = 'http://git.singsys.com/api/v3/projects/' + req.query.unique_identifierId + '/hooks/?private_token=hNaPq1UPYBmzRkKK25yH';
        request(urlHook, function(error, response, html) {
            if (error) {
                console.log('Error Has Occured' + error);
                var data = error;
...
router.get('/anshul', function(req, res) {
                        ...
                        sData.forEach(function(sData) {
                            ...
                            function execute() {
                                ...
                                if (createBranchFrom) {
                                    ...
                                    var command = 'cd "' + result.branches.pathonserver + '" && git clone -b ' + result.branches.branchname + ' http://automated:m07Q1CJ70LpMWm7L2y6y@git.singsys.com/' + result.group + '/' + result.unique_identifier + '.git';
                                    ...
                                    exit: function(exitCode) {
                                        if (exitCode != 0) {
                                            var options1 = {
                                                ...
                                                form: '{"attachments":[{"fallback":"Git Error While Making Directory[Urgent]","pretext":"Git Error While Making Directory[' + result.unique_identifier + ']","color":"danger","fields":[{"title":"Error Message`","value":"```' + errorMessage + '```","short":false}],"mrkdwn_in": ["text","pretext","fields"]}],"channel": "#git-error","parse":"none"}'
                                            };
                                ...
                                ssh.on('error', function(err) {
                                    ...
                                    if (count == 2) {
                                        var options1 = {
                                            uri: "https://hooks.slack.com/services/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",
                                            form: '{"attachments":[{"fallback":"Git Error Login[Urgent]","pretext":"Git Error While Making Directory[' + result.unique_identifier + ']","color":"danger","fields":[{"title":"Output","value":"```' + err + '```","short":false,"mrkdwn_in":true},{"title":"Error Message`","value":"' + err + '","short":false}],"mrkdwn_in": ["text","pretext","fields"]}],"channel": "#git-error","parse":"none"}'
                                        };
                                        request.post(options1, function(error, response, html) {
                                            console.log(html);
                                            return 1;
            ...
            ssh.on('error', function(err) {
                ...
                if (count == 2) {
                    var mailOptions = {
                        ...
                        to: user_email,
                        cc: 'shashwat@singsys.com, nirmal@singsys.com, nitish@singsys.com,prateekkaien@singsys.com',
                        subject: 'Code Sync Failed - Server Login Failed - Project: ' + appName + ', Branch: ' + currentBranch.name,
                        text: err.message,
